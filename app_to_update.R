...
server <- function(input, output){
  ...
  data <- reactive({mapData[mapData$Species %in% input$MapSpecies
                            & mapData$Length %in% input$MapLength
                            & mapData$Gear %in% input$MapGear
                            & mapData$VessNat %in% input$MapVessNat,]})
  ...
  output$selectMapLenght<- renderUI({
    checkboxGroupInput(inputId = "MapLength", label = "Select Lenght Group:",
                       as.list(l))
  })
